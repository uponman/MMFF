...
from utils.plots import output_to_target, plot_images, plot_val_study, plot_imagesd
...
def run(
        ...
        callbacks=Callbacks(),
        compute_loss=None,
        epoch=-1,  # 若epoch=-1则代表训练完成时画出混淆矩阵，否则则代表每个epoch结束时画出混淆矩阵
):
    # ——————————————————————————————————————————————————————————————————————
    #                       一、初始化配置
    # ——————————————————————————————————————————————————————————————————————
    ...
    if not training:
        ...
        taskd = taskd if taskd in ('traind', 'vald', 'test') else 'vald'
    ...
    for batch_i, (im, imd, targets, paths, pathsd, shapes, shapesd) in enumerate(pbar):
        callbacks.run('on_val_batch_start')
        with dt[0]:
            if cuda:
                im = im.to(device, non_blocking=True)
                imd = imd.to(device, non_blocking=True)
                targets = targets.to(device)
            im = im.half() if half else im.float()  # uint8 to fp16/32
            imd = imd.half() if half else imd.float()
            im /= 255  # 0 - 255 to 0.0 - 1.0
            imd /= 255
            nb, _, height, width = im.shape  # batch size, channels, height, width
            nbd, _, heightd, widthd = imd.shape

        # Inference
        with dt[1]:
            preds, train_out = model(im, imd) if compute_loss else (model(im, imd, augment=augment), None)
        ...
        for si, pred in enumerate(preds):
            ...
            correct = torch.zeros(npr, niou, dtype=torch.bool, device=device)  # init 第si张图片的地址
            seen += 1  # 统计测试图片数量 +1
        ...
        if plots and batch_i < 3:
            plot_imagesd(im, targets, paths, save_dir / f'val_batch{batch_i}_labels.jpg', names)  # labels
            plot_imagesd(im, output_to_target(preds), paths, save_dir / f'val_batch{batch_i}_pred.jpg', names)  # pred
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        fileclass = [save_dir / 'class1_results.csv', save_dir / 'class2_results.csv', save_dir / 'class3_results.csv', save_dir / 'class4_results.csv']
        ncol = 7
        for i, c in enumerate(ap_class):
            s = '' if fileclass[i].exists() else (('%15s,' * ncol % tuple(['epoch', 'Images', 'Instances', 'P', 'R', 'mAP50', 'mAP50-95'])).rstrip(',') + '\n')
            with open(fileclass[i], 'a') as f:
                f.write(s + ('%20.5g,' * ncol % tuple([epoch, seen, nt[c], p[i], r[i], ap50[i], ap[i]])).rstrip(',') + '\n')
    ...
    # Plots
    if plots:
        matrix = confusion_matrix.plot(save_dir=save_dir, names=list(names.values()), epoch=epoch)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    # print(matrix)
    return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t, matrix
...
def main(opt):
    ...
    if opt.task in ('train', 'traind', 'val', 'vald', 'test'):  # run normally
